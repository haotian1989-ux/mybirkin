...
import { CraftPageData, DEFAULT_CRAFT, CRAFT_KEYS, CRAFT_PAGE_LABELS } from "@/lib/craft-data";

function usePageData(): CraftPageData {
  const [data, setData] = useState<CraftPageData>(DEFAULT_CRAFT.overview);
  useEffect(() => {
    try {
      const raw = localStorage.getItem(CRAFT_KEYS.overview);
      if (raw) { setData(JSON.parse(raw)); return; }
    } catch {}
    const handler = () => {
      try {
        const r = localStorage.getItem(CRAFT_KEYS.overview);
