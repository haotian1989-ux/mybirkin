    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { setAdding(true); setEditing(emptyProduct); }}
          className="btn-primary text-[10px] gap-1 py-2 px-4"><Plus size={12} /> Add Product</button>
      </div>
      {(editing || adding) && (
    ...
    <div className="fixed inset-0 z-50 bg-charcoal/40 backdrop-blur-sm flex items-start justify-center pt-20 overflow-y-auto">
      <div className="bg-paper p-8 w-full max-w-2xl mx-4 shadow-2xl mb-20">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="col-span-2"><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Name</label><input value={form.name} onChange={(e) => upd("name", e.target.value)} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal" /></div>
          ...
          <div><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Category</label><select value={form.category} onChange={(e) => upd("category", e.target.value)} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal bg-paper"><option value="handbags">Handbags</option><option value="charms">Charms</option><option value="pet">Pet</option></select></div>
          <div><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Price ($)</label><input type="number" value={form.price} onChange={(e) => upd("price", Number(e.target.value))} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal" /></div>
          <div className="flex items-center gap-4"><label className="flex items-center gap-1.5 text-xs"><input type="checkbox" checked={form.featured} onChange={(e) => upd("featured", e.target.checked)} className="accent-charcoal" /> Featured</label><label className="flex items-center gap-1.5 text-xs"><input type="checkbox" checked={form.newArrival} onChange={(e) => upd("newArrival", e.target.checked)} className="accent-charcoal" /> New Arrival</label><label className="flex items-center gap-1.5 text-xs"><input type="checkbox" checked={form.inStock} onChange={(e) => upd("inStock", e.target.checked)} className="accent-charcoal" /> In Stock</label></div>
          <div className="col-span-2"><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Description</label><textarea value={form.description} onChange={(e) => upd("description", e.target.value)} rows={3} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal resize-none" /></div>
          <div className="col-span-2"><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Materials</label><input value={form.materials} onChange={(e) => upd("materials", e.target.value)} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal" /></div>
          <div className="col-span-2"><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Dimensions</label><input value={form.dimensions} onChange={(e) => upd("dimensions", e.target.value)} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal" /></div>
          <div className="col-span-2"><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Colors (comma separated)</label><input value={form.colors.join(", ")} onChange={(e) => upd("colors", e.target.value.split(",").map((s) => s.trim()).filter(Boolean))} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal" /></div>
          <div className="col-span-2"><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Image URLs (one per line)</label><textarea value={form.images.join("\n")} onChange={(e) => upd("images", e.target.value.split("\n").filter(Boolean))} rows={3} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal resize-none font-mono text-xs" /></div>
          <div className="col-span-2"><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Details (one per line)</label><textarea value={form.details.join("\n")} onChange={(e) => upd("details", e.target.value.split("\n").filter(Boolean))} rows={4} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal resize-none" /></div>
        </div>
        <div className="flex gap-3"><button onClick={() => onSave(form)} className="btn-primary py-2 px-6 text-xs"><Save size={13} className="mr-1.5" /> Save Product</button><button onClick={onCancel} className="btn-outline py-2 px-6 text-xs">Cancel</button></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="space-y-4 mb-8">
        <div><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Background Image URL</label><input value={hero.value.image} onChange={(e) => hero.save({ ...hero.value, image: e.target.value })} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal font-mono text-xs" />{hero.value.image && <div className="mt-2 aspect-[21/9] overflow-hidden bg-ivory/50"><img src={hero.value.image} alt="Preview" className="w-full h-full object-cover" /></div>}</div>
        <div><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Tagline</label><input value={hero.value.tagline} onChange={(e) => hero.save({ ...hero.value, tagline: e.target.value })} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal" /></div>
        <div><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Headline</label><textarea value={hero.value.headline} onChange={(e) => hero.save({ ...hero.value, headline: e.target.value })} rows={2} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal resize-none font-serif text-lg" /></div>
        <div><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Subtext</label><textarea value={hero.value.subtext} onChange={(e) => hero.save({ ...hero.value, subtext: e.target.value })} rows={3} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal resize-none" /></div>
        <div className="grid grid-cols-2 gap-4"><div><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Primary Button</label><input value={hero.value.primaryBtnLabel} onChange={(e) => hero.save({ ...hero.value, primaryBtnLabel: e.target.value })} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal" /></div><div><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Secondary Button</label><input value={hero.value.secondaryBtnLabel} onChange={(e) => hero.save({ ...hero.value, secondaryBtnLabel: e.target.value })} className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal" /></div></div>
      </div>
      <button onClick={saveAll} className={`btn-primary ${msg ? "bg-green-800 border-0" : ""}`}>{msg || "Save Homepage"}</button>
    </div>
  );
}
...
function ContactEditor() {
  ...
  return (
    <div className="max-w-lg">
      <h2 className="font-serif text-lg mb-1">Contact Links</h2><p className="text-xs text-smoke/60 mb-6">Floating contact button links</p>
      <div className="space-y-4 mb-6">
        <div><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">WhatsApp Link</label><input value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} placeholder="https://wa.me/1234567890" className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal font-mono text-xs" /></div>
        <div><label className="text-[10px] tracking-label uppercase text-smoke/50 block mb-1">Telegram Link</label><input value={telegram} onChange={(e) => setTelegram(e.target.value)} placeholder="https://t.me/yourusername" className="w-full border border-line px-3 py-2 text-sm focus:outline-none focus:border-charcoal font-mono text-xs" /></div>
      </div>
      <button onClick={save} className={`btn-primary ${saved ? "bg-green-800 border-0" : ""}`}>{saved ? "✓ Saved" : "Save Links"}</button>
    </div>
  );
}
